...
if (!videos.length) return conn.sendMessage(m.chat, { text: `${mess.nosear}` }, { quoted: m })
...
const v = videos[i]
caption += `> *${i + 1}* » ${v.title}\n`
caption += `⩩ *Duración* : ${v.timestamp || 'N/A'}\n`
caption += `⩩ *Publicado* : ${v.ago || 'N/A'}\n`
caption += `⩩ *Vistas* : ${toNum(v.views)}\n`
caption += `⩩ *Enlace* : ${v.url || 'N/A'}\n\n\n`
}
...
if (!apiData) return conn.sendMessage(m.chat, { text: mess.noapi }, { quoted: m })
