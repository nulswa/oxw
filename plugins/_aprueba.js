...
let handler = async (m, { conn, usedPrefix, command, text }) => {
    ...
    try {
        ...
        mensaje += `Has puesto en venta a *${personaje.name}*\n\n`;
        mensaje += `📋 *DETALLES DE LA PUBLICACIÓN:*\n`;
        ...
        mensaje += `💰 Precio original: ${precioOriginal} ToruCoins\n`;
        mensaje += `📉 Descuento aplicado: 30% (-${descuento} ToruCoins)\n`;
        mensaje += `💸 Precio de venta: ${precioVenta} ToruCoins\n\n`;
        mensaje += `🔖 Estado: En venta\n`;
        mensaje += `📅 Publicado: ${nuevaVenta.fechaPublicacion}\n\n`;
        mensaje += `_El personaje ha sido removido de tu colección y está disponible para que otros usuarios lo compren._\n`;
        mensaje += `_Recibirás ${precioVenta} ToruCoins cuando alguien lo compre._`;

        await conn.reply(m.chat, mensaje, m);

    } catch (error) {
        console.error(error);
        await conn.reply(m.chat, `❌ Error al poner en venta el personaje: ${error.message}`, m);
    }
};
