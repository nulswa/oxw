...
sendInteractiveList: {
  async value(jid, title, text, buttonText, sections, imageBuffer, quoted, options = {}) {
    ...
    // Si hay imagen, agregarla al header
    if (imageBuffer) {
      messageContent.interactiveMessage.header = {
        title: title,
    ...
    // Enviar el mensaje
    const msg = await conn.relayMessage(
      jid,
