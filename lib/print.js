...
console.log(`
┌────────────────
│⫹⫺ ${chalk.greenBright('Servidor:')} ${chalk.cyanBright(me)} ~ ${chalk.blueBright(userName)} ${global.conn.user.jid === conn.user.jid ? chalk.cyanBright('(Principal)') : chalk.cyanBright('(Server.)')}
│⏍ ${chalk.greenBright('Fecha:')} ${chalk.cyanBright(new Date(m.messageTimestamp ? 1000 * (m.messageTimestamp.low || m.messageTimestamp) : Date.now()).toLocaleDateString("es-ES", { timeZone: "America/Mexico_City", day: 'numeric', month: 'long', year: 'numeric' }))}
│々 Remitente: ${chalk.greenBright(sender)}
│⎋ Chat: ${chalk.cyanBright(`${m.isGroup ? 'Grupal' : 'Privado'} >`)} ${chalk.greenBright(chat)}
│⫶☰ Tipo de evento: ${chalk.greenBright(m.messageStubType ? WAMessageStubType[m.messageStubType] : 'Ninguno.')}
└────────────────`)
