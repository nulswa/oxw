...
const headers = {
    ...
    'Product-Serial': '72405aaeae4d6fcbbe71854be3d00603', 
...
async function solveTurnstile() {
    try {
        const response = await axios.post('https://bypasscf.ryzecodes.xyz/api/bypass/cf-turnstile', {
            url: BASE_URL,
            siteKey: SITE_KEY
        }, {
...
async function createJob(imageBuffer, prompt, retry = false) {
    ...
    try {
        ...
        throw error
        //new Error(`📍  Limite de edición agotada por usuarios.\n- Vuelva a usar el comando pronto...`)
    }
}
...
async function getJob(jobId) {
    ...
    while (attempts < 30) {
        try {
            ...
            if (jobStatus === -1 || jobStatus === 'failed') { 
                throw new Error(`📍  No se pudo leer el prompt...`)
            }
    ...
    throw new Error(`📍  Tiempo de edición tardado.\n- Reporte el comando...`)
}
...
return conn.sendMessage(m.chat, { text: `*[ ⽷ ]*  Este comando es exclusivo para el plan *[ Premium ]*\n- Usa *#plan* para ver los planes disponibles.` }, { quoted: m })
    ...
    if (!/image\/(jpe?g|png)/.test(mime)) {
       return conn.sendMessage(m.chat, { text: `ᗢ Responda a una imagen y proporciona un texto para editar la imagen.\n\n${mess.example}\n*${usedPrefix + command}* Pon un bosque como fondo en esta imagen..` }, { quoted: m });
    }

    const prompt = text.trim()
    if (!prompt) return conn.sendMessage(m.chat, { text: `Debes ingresar un prompt para editar la imagen.` }, { quoted: m });
    ...
    try {
        ...
        if (!jobId) {
             ...
             if (jobData.code && jobData.code !== 100000) throw new Error(`📍  Limite de edición agotada por usuarios.\n- Vuelva a usar el comando pronto...`)
        ...
        if (!outputUrl) throw new Error(`${mess.noapi}`)

        const mediaResponse = await axios.get(outputUrl, { responseType: 'arraybuffer' })
        
        await conn.sendMessage(m.chat, { image: mediaResponse.data, caption: null }, { quoted: m })
