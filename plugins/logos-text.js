...
if (!modelo) {
return conn.sendMessage(m.chat, { text: `${emoji}  No se ha encontrado el logo ( *${usedPrefix + command}* ), verifica si esta en la lista de logos.` }, { quoted: m });
};

const data = await ephoto(modelo, text);
await conn.sendMessage(m.chat, { image: { url: data }, caption: `${mess.succs}\n\n\t＃ *Texto* : ${text}\n\t＃ *Fuente* : ephoto360\n\n${botname}\n> ${textbot}` }, { quoted: m });
};
